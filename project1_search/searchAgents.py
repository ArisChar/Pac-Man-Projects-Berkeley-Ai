...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, self.expanded_corners)
        ...
        for action in self.getActions(state):
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextx, nexty) in self.corners:
                    traversed_corner[self.corners.index((nextx,nexty))] = 1

                children.append((((nextx, nexty), traversed_corner), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for corner in traversed_corner:
        if corner == 0:
            heuristic = max(heuristic, manhattanDistance(state[0],corners[index]))
        index +=1
